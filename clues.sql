...
JOIN public.countrylanguages cl ON c.code = cl.countrycode
WHERE cl.language = 'Italian'
GROUP BY c.name
HAVING COUNT(cl.language) = 1 AND
       SUM(CASE WHEN cl.isofficial = TRUE THEN 1 ELSE 0 END) = 1
       AND c.name != 'Switzerland' AND c.name != 'Holy See (Vatican City State)' AND c.name != 'Italy';
...
JOIN public.countries co ON ci.countrycode = co.code
WHERE ci.name LIKE 'Serra%' AND co.continent = 'South America';
...
JOIN public.cities ci ON co.capital = ci.id
